...
### PL-1 — Off-Domain-Link: „Referenzen"-CTA zeigt auf die alte GitHub-Pages-Staging-Domain
...
### PL-2 — Calendly-Consent-Box: Hinweistext praktisch unlesbar (Kontrast 1.57:1)
...
### PL-3 — Team-Seite live mit Platzhalter-Inhalten
...
## 🔵 MINOR
- **PL-4 — EN-Zahlenformat:** englische Prosa nutzt deutsches Tausender-Format „1.600 mm / 1.300 mm" (inkonsistent: hand-getextete EN-Startseite nutzt korrekt „1,800 mm"). Auch die **Hero-Tiefenskala zeigt „3.000 mm" auf EN-Seiten** (per `aria-hidden` für AT versteckt, aber sichtbar). Quelle `maschinenseiten.js` / `index.njk`. *(MI-14)*
- **PL-5 — heading-order:** `unternehmen.html` springt h2→h4 (4 Prinzip-Karten; indexierte Seite). `impressum.html` h1→h3 (noindex, vernachlässigbar). → Karten-Headings auf `<h3>` setzen, Größe per CSS halten.
- **PL-6 — Redundante Redirect-Zeile:** `_redirects` hat `/aktuelles/` zusätzlich zur Splat-Regel `/aktuelles/*` (unerreichbar, gleiches Ziel). Kosmetisch; Netlify-Warnung möglich. Quelle `src/_redirects.njk`.
- **PL-7 — EN-Privacy-Checkbox `value="akzeptiert"`** (deutsch). Nicht sichtbar, Einwilligung wird trotzdem übertragen; kosmetisch.
...
- **Maschinenseiten-LCP ~7,7 s (mobil gedrosselt)** — größter verbliebener Hebel: Page-Hero ist ein **CSS-`background-image`** (kein `<picture>`, **kein Preload, kein WebP**, 388 KB JPG obwohl `.webp`-Twin existiert) auf allen 60 Seiten. → pro Seite WebP preloaden bzw. Hero als echtes `<img>`/`<picture>`.
...
- `og:type=website` auf Maschinenseiten (könnte `product`).
