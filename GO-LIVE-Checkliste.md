...
- [ ] **Calendly-Konto ersetzen** — die Terminbuchung auf `/kontakt.html` zeigt auf `calendly.com/**luka-bloemendal**/30min` (`src/kontakt.njk:72`), also auf ein privates Konto des Erstellers, nicht auf LIBA. Vor Go-Live durch ein LIBA-Konto ersetzen — oder die Terminbuchung ganz entfernen. **[Betreiber liefert Konto/URL → Repo]** · gefunden bei der unabhängigen Prüfung 2026-07-27
...
- [x] **CSP: Calendly war auf Netlify blockiert** — erledigt 2026-07-27. CSP-Wildcards decken die Apex-Domain **nicht** ab: `frame-src https://*.calendly.com` erlaubt `assets.calendly.com`, aber nicht `calendly.com` — genau von dort lädt das Widget seinen iframe. Die Terminbuchung hätte nach dem Consent-Klick ein leeres Feld gezeigt, und zwar erst auf Netlify (lokal und auf der Pages-Vorschau werden keine Header ausgeliefert, deshalb war es nie aufgefallen). `https://calendly.com` jetzt in `script-src`, `frame-src` und `connect-src`; zusätzlich `assets.calendly.com` in `style-src`, weil das Widget sein Stylesheet nachlädt; und `analytics.google.com` als Apex ergänzt. **Auf dem Netlify-Deploy-Preview gegenprüfen.**
